...
static int  end_command;   /* Controla o fim de execu��o do interpretador de comandos */
...
static int  history_pos;  /* Posi��o para navega��o no hist�rico */
/* Vari�veis para defini��o de vari�veis internas */
...
static command_t commands[MAXCOMMANDS] = {
  ...
  "startBarber"," <pid>   Inicia a Barbearia",					cmd_start_barber,
  "stopBarber"," <pid>    Finaliza a Barbearia",				cmd_stop_barber,
  "addClienteBarber"," <pid> Adiciona Cliente",					cmd_add_cliente_barber
};
...
void command()
{
  ...
  setcolor(LOGOCOLOR);
  putstr("                  �������  ������\n");
  putstr(" �����  �  ���  � ��   ��  ��  ��  "); setcolor(7); putstr("MinisSistema Operacional\n"); setcolor(LOGOCOLOR);
  putstr("�� � � �� �� � �� ������� ���  ��  "); setcolor(7); putstr("Vs "); putstr(miniSO_VERSION); putstr("\n"); setcolor(LOGOCOLOR);
  ...
  command_showprompt();
  /* La�o para interpreta��o de comandos */
  while ( !end_command )  {
        car=getch();
        switch (car)  {
               case 9:
               ...
               case 13:
                    ...
                    putch('\n');
                    cmdline[numcar]='\0';
                    /* Divide a linha de comandos em comando e op��es */
                    ptrline = cmdline;
                    argc = 0;
                    pos = 0;
                    while (1) {
...
int cmd_ps(int argc, char far *argv[])
{
  ...
  argc=argc;
  argv=argv;
  putstr(" BCP   PID   PPID  STATUS   ZLIST  PREV   NEXT\n");
...
static void startBarbeiro(void);
...
int count=0;
int numCadeiras = 3;
semid_t mutex, clientes, fila;

int cmd_start_barber (int argc, char far *argv[]) {
	
	fila = semcreate(0);
	clientes = semcreate(0);
	mutex = semcreate(1);

	putstr("*** BARBEARIA INCIADA COM SUCESSO ***\n");
	if	(fork(startBarbeiro)==miniSO_ERROR)  {
		putstr("Erro em demo: fork() nao conseguiu criar thread!\n");
		return 1;
	}
...
int cmd_stop_barber  (int argc, char far *argv[]){
	
	pid_t pid=0;
	if (argc < 2){
		putstr("stopBarber: Favor informar o PID da barbearia!\n");
		return 1;	
	}
	pid = atoi(argv[1]);
	kill(1);
...
int cmd_add_cliente_barber (int argc, char far *argv[]){
	
	semdown(mutex);
	if (count < 3) {
		count ++;
		semup(clientes); // Acorda o barbeiro
		semup(mutex);
		semdown(fila); // Espera o barbeiro
	} else {
		semup(mutex);
	}
